...
PresentationDuration_list = [0.1, 0.06, 0.04, 0.03, 0.02, 0.01]; %---- the duration of each stimuli before mask onset.
NTrials_ThisDuration = [10, 25, 25, 25, 25, 25];  %--- number of trials for each PresentationDuration for a condition

if DemoOrPracticeOrTest==1
    disp('Demo only');
    StimulusChoices_ThisSession_From_Prob_list  = 1:length(Prob_lists);
    PresentationDuration =input('enter duration of the stimulus display, 0.1 second, 4 second, e.g.');
    PresentationDuration_list = [PresentationDuration];
    NTrials_ThisDuration =[1];
    NBreaks = 1;
elseif DemoOrPracticeOrTest==2
    StimulusChoices_ThisSession_From_Prob_list  = [1, 2];  %--- just the homo pairs, left tilted and right tilted. 
    NTrials_ThisDuration = [10, 25, 25, 25, 25, 25]; 
    disp('Practice session');
    NBreaks = 1;
else
    disp('Testing session');
    StimulusChoices_ThisSession_From_Prob_list  = 1:length(Prob_lists); %--- choose the ones from Prob_list as stimuli for this session.
    PresentationDuration_list = [0.1, 0.06, 0.04, 0.03, 0.02, 0.01]; %---- the duration of each stimuli before mask onset.
    NTrials_ThisDuration = [10, 25, 25, 25, 25, 25];  %--- number of trials for each PresentationDuration for a condition    
    NBreaks = 3;
end
...
for i = 1:length(PresentationDuration_list)
    for j = 1:length(StimulusChoices_ThisSession_From_Prob_list)
        ...
        condition.AddMask=1;
%         condition.AddMask_Delay = 0.05; %--- this is never used again in the code.
        condition.MaskDuration = 0; %---
        condition.Nrepeat_MaskAndStimulus =1;
        condition.N_masks = 1;
        count  = count+1;
        Conditions(count) = condition;
        NTrials_EachCondition = [NTrials_EachCondition, NTrials_ThisDuration(i)];
    end
end
