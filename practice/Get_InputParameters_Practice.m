...
NbX = 4; % Itemsize= StimulusSquareSideLength/NbX
NbY = 4;
if NbX ~= NbY
    error('NbX and NbY are unequal to each other, not suitable for this project');
end
Disk_Radius_in_Scale = 0.05; % used as a radius for the square side length
...
FixationCenter_y_in_Scale = [0,    -0.1491];


N_Locations =length(StimulusCenter_x_in_Scale);  


PresentationDuration_list = [0.2]; %---- the duration of each stimuli before mask onset.




%NTrials_ThisDuration = [20];  %--- number of trials for each PresentationDuration for a condition
...
InputParameters.FontSize_To_Sizebar = 1/0.5;
